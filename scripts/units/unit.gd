...
func move_towards(opponent_node) -> void:
	...
	if opponent != null and is_instance_valid(opponent) and opponent.global_position.x < global_position.x:
...
func _physics_process(_delta: float) -> void:
	...
	# While in combat, if the opponent died/freed, unlock and return home.
	if in_combat:
		if opponent == null or !is_instance_valid(opponent) or opponent.dead:
			_end_combat_and_return()
		return
	...
	# If we have nothing to do, disappear (matches original behavior).
	if opponent == null and !returning:
		queue_free()
		return

	# Opponent might have been freed.
...
func _end_combat_and_return() -> void:
	in_combat = false
	_combat_loop_running = false
	opponent = null
...
func _lock_combat() -> void:
	...
	# Lock opponent too (only if they aren't already in combat).
...
func _lock_combat_from(other: Unit) -> void:
...
func _wait_both_attack_finished() -> void:
	while true:
		...
		if opponent != null and is_instance_valid(opponent) and opponent.animation != null:
...
func defeat() -> void:
	...
	# Tell opponent to stop being stuck in combat.
	var opp := opponent
	opponent = null
	if opp != null and is_instance_valid(opp) and !opp.dead:
		opp._end_combat_and_return()
